...
In Figure \@ref(fig:pbmc-box-purities), median purity values almost all greater than 0.9, indicating that most cells in each cluster are primarily surrounded by other cells from the same cluster.
...
```{r, echo=FALSE}
by.cluster <- split(pure.data$purity, pure.data$cluster)
med.by.clust <- vapply(by.cluster, median, 0)
stopifnot(all(med.by.clust > 0.89))
```
...
```{r, echo=FALSE}
which.min(sf.by.clust) %in% head(order(rmsd, decreasing = TRUE), n = 2)
```
