...
In the table below, each row corresponds to one cluster, and large counts indicate that its cells are easily confused with those from another cluster.
This provides another perspective on the relationships between clusters. 
...
The aim is to find the set of parameter values that maximizes the average silhouette width in order to obtain well-separated clusters.
...
A related metric to assess cluster separation is the degree to which cells from multiple clusters intermingle in expression space.
...
```{r pbmc-box-purities, eval=FALSE, fig.wide=TRUE, fig.cap="Distribution of cluster purities across cells in each cluster of the PBMC dataset. Each point represents a cell and colored with the identity of the cluster contributing the largest proportion of its neighbors."}
...
However, it can be computed for any set of clusters if the original coordinates are available.

```{r, eval=FALSE}
kmeans.info <- clusterCells(sce.pbmc, use.dimred="PCA", 
    BLUSPARAM=KmeansParam(center=10), full=TRUE)
clust.kmeans2 <- kmeans.info$objects

# TODO: add bluster function to generate these statistics.
...
A cluster is more likely to have a low RMSD if it has no internal structure and is separated from other clusters.
If two clusters are poorly separated, there is likely to be many cells on the boundaries between clusters, which would result in a higher sum of squares from the centroid.
Thus, we can diagnose poorly-defined clusters as those with relatively large per-cluster RMSDs.
-->
...
The simplest approach for comparing two clusterings of the same dataset is to create a 2-dimensional table of label frequencies (Figure \@ref(fig:walktrap-v-others)).
We can then identify how our Walktrap clusters from Section \@ref(clustering-graph) are redistributed when we switch to using Louvain community detection (Figure \@ref(fig:walktrap-v-louvain-prop)).
...
jacc.mat <- linkClustersMatrix(clust.walktrap, clust.louvain, denominator="union")
...
```{r pbmc-graph-linked, fig.cap="Force-directed layout of the graph of the clusters obtained from different variants of community detection on the PBMC dataset."}
...
```{r kmeans-gap, eval=FALSE, fig.cap="Gap statistic with respect to increasing number of $k$-means clusters in the 10X PBMC dataset. The red line represents the chosen $k$."}
library(cluster)
set.seed(110010101)
