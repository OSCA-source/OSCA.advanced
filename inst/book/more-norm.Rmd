...
An interesting subtlety of `logNormCounts()` is that it will center the size factors at unity.
This puts both the pseudo-count and the normalized expression values on the same scale such that a pseudo-count of 1 can be interpreted as an extra read or UMI for each gene.
...
Conversely, at lower sequencing depths, stronger shrinkage avoids spuriously large log-fold changes between subpopulations. 
...
This is a cruel irony given that the whole intention of sequencing more deeply is to improve quantification of these differences between cell subpopulations. 
...
One may also see the inverse hyperbolic sine (a.k.a, arcsinh) transformation, which is very similar to the log-transformation when considering non-negative values.
