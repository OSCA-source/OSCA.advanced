...
One property of the AUC is that it is symmetric with respect to differences in the size of the groups being compared.
All else being equal, the genes with the largest AUCs on either side of a comparison will have similar expression profiles regardless of the number of cells in each group.
In contrast, Cohen's $d$ will favor genes where the larger group has the smaller relative variance as this results in a lower pooled variance.
This is not necessarily a bad outcome for marker detection, but it does require some thought to understand the differences in the expression profiles across groups of different sizes.
Some of these effects are observed in a comparison between alpha and gamma cells in the human pancreas data set from @lawlor2017singlecell (Figure \@ref(fig:comparative-markers-tw)).
...
marker.pancreas <- scoreMarkers(subset, groups=subset$`cell type`)

# Upregulated in alpha:
marker.alpha <- marker.pancreas$Alpha
chosen.alpha.lfc <- rownames(marker.alpha)[head(order(marker.alpha$mean.logFC.cohen, decreasing=TRUE), 20)]
chosen.alpha.auc <- rownames(marker.alpha)[head(order(marker.alpha$mean.AUC, decreasing=TRUE), 20)]
u.alpha.lfc <- setdiff(chosen.alpha.lfc, chosen.alpha.auc)
u.alpha.auc <- setdiff(chosen.alpha.auc, chosen.alpha.lfc)

# Upregulated in gamma:
marker.gamma <- marker.pancreas$`Gamma/PP`
chosen.gamma.lfc <- rownames(marker.gamma)[head(order(marker.gamma$mean.logFC.cohen, decreasing=TRUE), 20)]
chosen.gamma.auc <- rownames(marker.gamma)[head(order(marker.gamma$mean.AUC, decreasing=TRUE), 20)]
u.gamma.lfc <- setdiff(chosen.gamma.lfc, chosen.gamma.auc)
u.gamma.auc <- setdiff(chosen.gamma.auc, chosen.gamma.lfc)

# Examining all uniquely detected markers in each direction.
library(scater)
gridExtra::grid.arrange(
    plotExpression(subset, x="cell type", features=u.alpha.lfc, ncol=2) +
        ggtitle("Upregulated in alpha, Cohen-only"),
    plotExpression(subset, x="cell type", features=u.alpha.auc, ncol=2) +
        ggtitle("Upregulated in alpha, AUC-only"),
    plotExpression(subset, x="cell type", features=u.gamma.lfc, ncol=2) +
        ggtitle("Upregulated in gamma, Cohen-only"),
    plotExpression(subset, x="cell type", features=u.gamma.auc, ncol=2) +
        ggtitle("Upregulated in gamma, AUC-only"),
    ncol=2
)
```

Cohen's $d$ is useful as it accounts for the magnitude of the change in expression.
...
two perfectly separated distributions will have an AUC of 1 regardless of the difference in means (Figure \@ref(fig:simulated-auc-cohen)).
This reduces the resolution of the ranking and makes it more difficult to distinguish between good and great markers.

```{r simulated-auc-cohen, echo=FALSE, fig.width=8, fig.height=4, fig.cap="Distribution of log-expression values for two simulated genes in a pairwise comparison between clusters. Both genes have the same AUC of 1 but the first gene has a larger Cohen's $d$ than the second."}
set.seed(100)
y0 <- c(2^rnorm(100, 5))
y1 <- 2^c(rnorm(100, 5, 0.8), rnorm(100, 1, 0.8))
...
plotExpression(SCE, x="cluster", rownames(SCE))
```
