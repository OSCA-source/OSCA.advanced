...
While t-SNE is a very useful dimensionality reduction technique for single cell
data, initial algorithms scale poorly with the number of cells. To this end,
fast interpolation-based t-SNE (FIt-SNE) was developed [@linderman2019fitsne].
This algorithm uses *interpolation nodes* to reduce the computational complexity
of the calculations from $N\log N$ to $\sim 2 p N$. This algorithm is 
implemented in `r Biocpkg("snifter")`, which calls the Python
library [`openTSNE`](https://opentsne.readthedocs.io/) using 
`r Biocpkg("basilisk")`. As Figure \@ref(fig:snifter-scater) shows, the 
embeddings produced by this method are qualitatively similar to those produced
by other algorithms; in fact, there
@linderman2019fitsne demonstrate that the error with respect to the embeddings
produced by previous t-SNE implementations is low and bounded.
To use this method, we can simply set `use_fitsne=TRUE` when calling 
`runTSNE` with `r Biocpkg("scater")`.
...
Using `r Biocpkg("snifter")` directly, we can also take advantage of another
feature of the `openTSNE` library. This library allows users to create an
embedding and to project new points into this existing embedding.
In this process, the existing points remain static, and new points are projected
based on their affinities with each other and the points in the existing 
embedding. In Figure \@ref(fig:snifter-embedding), it's clear that, in general,
cells are projected near to cells of a similar type.
This may be useful as a qualitative or exploratory step when combining 
datasets, though should not be used as the basis for empirical results.
...
A potential downside of t-SNE and UMAP is that they preserve the neighbourhood
structure of the data while neglecting the local density of the data.
This can result in seemingly compact clusters on a t-SNE or UMAP
plot that correspond to very heterogeneous groups in the original data.
The algorithms dens-SNE and densMAP were introduced to improve this somewhat,
by incorporating information about the average distance to the nearest
neighbours when creating the embedding [@narayan2021densvis].
Figures \@ref(fig:densne) and \@ref(fig:densmap) show that these methods provide
more information about transcriptional heterogeneity within clusters,
with astrocytes, endothelial and microglia clusters less compact in
the density-preserving versions.
This excessive compactness can imply a lower level of within-population
heterogeneity.
...
```{r densmap, fig.width=10, fig.height=6, fig.cap="UMAP and densMAP embeddings for the Zeisel brain data."}
gridExtra::grid.arrange(
    plotReducedDim(sce.zeisel, "UMAP", colour_by="level1class") + ggtitle("t-SNE"),
