...
see Section \@ref(big-data-svd) for more details - though the default IRLBA will suffice for more common in-memory representations.
...
To avoid this, we avoid explicit calculation of the residuals during matrix multiplication (see `?ResidualMatrix` for details), allowing us to perform approximate PCA more efficiently.
...
See `r link("merged-pancreas", "OSCA.workflows")` for an example of a more complex `fastMNN()` merge involving several human pancreas datasets generated by different authors on different patients with different technologies.
...
This focuses on the relevant heterogeneity and represents a semi-supervised approach that is a natural extension of the strategy described in `r link("apriori-hvgs", "OSCA.basic")`.

To illustrate, we apply this strategy to our PBMC datasets.
We identify the top marker genes from pairwise Wilcoxon ranked sum tests between every pair of clusters _within_ each batch, analogous to the method used by `r Biocpkg("SingleR")` (`r link("cell-type-annotation", "OSCA.basic")`).
...
Rather, it is preferable to perform DE analyses using the uncorrected expression values with blocking on the batch, as discussed in `r link("marker-batch", "OSCA.basic")`.
